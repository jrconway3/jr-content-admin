...
function jr_content_admin_render_repeater_wrapper( $title, $description, $rows_html, $add_label, $template_id ) {
	...
	foreach ( $rows_html as $row_html ) {
		echo $row_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML generated by this plugin; individual values already escaped in row renderers.
	}
	echo '</div>';
